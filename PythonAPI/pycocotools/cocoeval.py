...
class COCOeval:
    ...
    def summarize(self):
        ...
        def _summarizeDets():
            ...
            # Extra metrics
            stats[13] = _summarize(0, iouThr=.5, maxDets=self.params.maxDets[2])
            stats[14] = _summarize(0, iouThr=.7, maxDets=self.params.maxDets[2])
            stats[15] = _summarize(0, iouThr=.85, maxDets=self.params.maxDets[2])
...
class Params:
    ...
    def setKpParams(self):
        ...
        self.recThrs = np.linspace(.0, 1.00, int(np.round((1.00 - .0) / .01)) + 1, endpoint=True)
        self.maxDets = [20]
        self.areaRng = [[0 ** 2, 1e5 ** 2], [0, 32 ** 2], [32 ** 2, 96 ** 2], [96 ** 2, 1e5 ** 2]]
        self.areaRngLbl = ['all', 'small', 'medium', 'large']
        self.useCats = 1
